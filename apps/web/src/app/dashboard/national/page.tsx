...
export default function NationalDashboard() {
  ...
  useEffect(() => {
    const loadNationalData = async () => {
      try {
        ...
        const [currentUser, nData] = await Promise.all([
          authService.getCurrentUser().catch(() => session.user),
          dashboardService.getNational().catch((err) => {
            console.warn('National dashboard API error:', err);
            return null;
          }),
        ]);
        ...
        console.error('National dashboard load error:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleGenerateCabinetNote = () => {
    const note = `GOVERNMENT OF INDIA - CABINET SECRETARIAT
DEPARTMENT OF LAND RESOURCES (DoLR) / PM GATI SHAKTI
CONFIDENTIAL CABINET NOTE ON NATIONAL LAND ACQUISITION
Date: ${new Date().toLocaleDateString('en-IN')}
------------------------------------------------------------
1. Central Project Pipeline: ${data?.nationalKpis?.centralPipeline ?? 284} Projects
2. Aggregate Land Footprint: ${data?.nationalKpis?.aggregateLandHectares ?? 1420000} Ha (Across 24 States/UTs)
3. Central Escrow Disbursed: ₹${data?.nationalKpis?.escrowDisbursedCr ?? '1.2L'} Cr
4. Stalled / Capital Blocked: ₹${data?.nationalKpis?.capitalBlockedCr ?? '1.8L'} Cr
------------------------------------------------------------
INTER-MINISTERIAL RECOMMENDATION:
Expedite Stage-II Forest Clearances in Western Corridor (MP/MH).
Direct MoEFCC & DoLR joint review with PMO next Monday.`;

    const blob = new Blob([note], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Cabinet_Note_DoLR_${new Date().toISOString().slice(0, 10)}.txt`;
  ...
  const kpis = data?.nationalKpis || {
    centralPipeline: 284,
    aggregateLandHectares: 1420000,
    escrowDisbursedCr: '1.2L',
    capitalBlockedCr: '1.8L',
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-serif font-bold text-[#122C4A]">
                  {t('national.title', 'National Master Monitoring System')}
                </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <span className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                {t('national.kpi_pipeline', 'Central Project Pipeline')}
              </span>
              <span className="text-4xl font-serif font-bold text-[#122C4A]">
                {kpis.centralPipeline}
              </span>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <span className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                {t('national.kpi_footprint', 'Aggregate Land Footprint')}
              </span>
              <span className="text-4xl font-serif font-bold text-[#122C4A]">
                1.42M <span className="text-lg font-sans text-gray-400">Ha</span>
              </span>
              <span className="text-xs text-green-600 font-semibold mt-2 block">
                Across 24 States / UTs
              </span>
            </div>

            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <span className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                {t('national.kpi_escrow', 'Central Escrow Disbursed')}
              </span>
              <span className="text-4xl font-serif font-bold text-[#122C4A]">
                ₹{kpis.escrowDisbursedCr} <span className="text-lg font-sans text-gray-400">Cr</span>
              </span>
              <span className="text-xs text-green-600 font-semibold mt-2 block">
                FY 2026-27 Till Date
              </span>
            </div>

            <div className="bg-[#0B1F35] border border-[#1D5FA8] rounded-xl p-6 shadow-sm relative overflow-hidden">
              <svg
                className="absolute right-0 bottom-0 w-24 h-24 text-white opacity-5"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <span className="block text-xs font-bold text-[#F2A71B] uppercase tracking-wider mb-2">
                {t('national.kpi_blocked', 'National Capital Blocked')}
              </span>
              <span className="text-4xl font-serif font-bold text-white">
                ₹{kpis.capitalBlockedCr} <span className="text-lg font-sans text-gray-400">Cr</span>
              </span>
              <span className="text-xs text-[#9FB0C4] mt-2 block">
                Stalled by litigation / clearances
              </span>
            </div>
          </div>

          {/* INTERSTATE CORRIDOR COMMAND */}
            ...
            <div className="px-8 py-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-serif font-bold text-[#122C4A]">
                  Interstate Infrastructure Matrix (PM Gati Shakti)
                </h2>
              ...
              <table className="w-full text-left border-collapse min-w-300">
                <thead>
                  <tr className="bg-white text-gray-500 text-[11px] uppercase tracking-wider border-b border-gray-200">
                    ...
                    <th className="px-6 py-4 font-bold w-[45%]">
                      State-Wise Acquisition Progress (Sec 19 Declarations)
                    </th>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-red-500 h-2 rounded-full w-[64%]"></div>
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between text-[10px] font-bold mb-1">
                            <span className="text-gray-600">GJ</span>
                            <span className="text-[#F2A71B]">81%</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-[#F2A71B] h-2 rounded-full w-[81%]"></div>
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between text-[10px] font-bold mb-1">
                            <span className="text-gray-600">MH</span>
                            <span className="text-[#F2A71B]">78%</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-[#F2A71B] h-2 rounded-full w-[78%]"></div>
                          </div>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-[10px] font-bold mb-1">
                            ...
                            <span className="text-[#F2A71B]">88%</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-[#F2A71B] h-2 rounded-full w-[88%]"></div>
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between text-[10px] font-bold mb-1">
                            <span className="text-red-600">WB</span>
              ...
              <div className="px-6 py-4 border-b border-gray-200 bg-red-50 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-bold text-red-800">
                    Inter-Ministerial Logjam Queue
                  </h2>
                </div>
                ...
                <ul className="divide-y divide-gray-100">
                  <li className="p-4 hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-[10px] font-bold text-[#122C4A] bg-gray-200 px-2 py-0.5 rounded">
                        MoEFCC
                      </span>
                    </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col">
              <div className="px-6 py-4 border-b border-gray-200 bg-[#FDF8E3] flex justify-between items-center">
                <h2 className="text-base font-bold text-[#122C4A]">
                  State Land Acquisition Index (SLAI)
                </h2>
              ...
              <div className="p-6 flex-1 flex flex-col justify-center">
                ...
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-bold text-[#1B2430] flex items-center gap-2">
                      <span className="bg-green-100 text-green-700 px-2 py-0.5 text-xs rounded">#1</span>{' '}
                      Madhya Pradesh
                    </span>
                    <span className="text-xs font-bold text-gray-500">
                      Escrow Disbursal: <span className="text-green-600">92%</span>
                    </span>
                  </div>
                  <p className="text-xs text-gray-500">
                    Lowest statutory turnaround time (avg. 280 days). Full DILRMP integration complete.
                  </p>
                </div>
                ...
                <div className="mb-6 pt-4 border-t border-gray-100">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-bold text-[#1B2430] flex items-center gap-2">
                      <span className="bg-gray-100 text-gray-600 px-2 py-0.5 text-xs rounded">#12</span>{' '}
                      Maharashtra
                    </span>
                    <span className="text-xs font-bold text-gray-500">
                      Escrow Disbursal: <span className="text-[#F2A71B]">68%</span>
                    </span>
                  </div>
                  <p className="text-xs text-gray-500">
                    Moderate R&R compliance. ₹4,500 Cr central funds currently idle in state treasury.
                  </p>
                </div>
                ...
                <div className="pt-4 border-t border-gray-100">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-bold text-[#1B2430] flex items-center gap-2">
                      <span className="bg-red-100 text-red-700 px-2 py-0.5 text-xs rounded">#27</span>{' '}
                      West Bengal
                    </span>
                    <span className="text-xs font-bold text-gray-500">
                      Escrow Disbursal: <span className="text-red-500">31%</span>
                    </span>
                  </div>
                  <p className="text-xs text-gray-500">
                    Severe delays in Sec 19 declarations. Frequent localized litigations stalling central corridors.
                  </p>
                </div>
              </div>
            </div>
