...
export default function CentralHeader({
  ...
  const navLinks = [
    { name: '🏛️ Apex Command', href: '/dashboard/national' },
    { name: 'Projects Pipeline', href: '/projects' },
    { name: 'Requiring Bodies', href: '/organizations' },
    { name: 'Interstate Corridors', href: '/gis/projects' },
    { name: 'Satellite Masterplan', href: '/gis' },
    { name: 'Central Escrow', href: '/compensation' },
    { name: 'Workflow Approvals', href: '/workflow/approvals' },
    { name: 'Cabinet Reports', href: '/reports' },
  ...
  return (
    <header className="w-full bg-white font-sans sticky top-0 z-50 shadow-sm">
      ...
      <div className="max-w-[1700px] mx-auto px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-4">
        {/* Ministry Emblem & Branding */}
        <div className="flex items-center gap-4">
          <Link href="/dashboard/national" className="flex items-center gap-4 group">
            <img
        ...
        <div className="flex items-center flex-wrap gap-3">
          {/* Live Data Grid Status */}
          <div className="hidden xl:flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full text-[11px] text-emerald-800 font-semibold">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            National Land Grid Active
          </div>
          ...
          <div className="flex items-center gap-2.5 pl-3 border-l border-[#DDD8C8]">
            <div className="flex items-center gap-2">
              <span className="w-8 h-8 rounded-full bg-[#122C4A] text-amber-300 font-bold text-xs flex items-center justify-center ring-1 ring-amber-400">
                {user.name ? user.name[0].toUpperCase() : 'C'}
              </span>
              <div className="hidden md:flex flex-col text-left">
                <span className="text-xs font-bold text-[#122C4A] leading-tight">{user.name}</span>
                <span className="text-[10px] text-[#B96E22] font-semibold">National Commissioner</span>
              </div>
            </div>
            <button
              onClick={onLogout}
      ...
      <nav className="bg-[#122C4A] text-white px-6 lg:px-8 py-0 shadow-inner">
        <div className="max-w-[1700px] mx-auto flex items-center justify-between overflow-x-auto">
          ...
          <div className="hidden lg:flex items-center gap-3 pl-4 py-1 text-xs text-amber-300">
            <span className="text-[11px] font-mono">DoLR Apex Gateway</span>
          </div>
        </div>
      </nav>
    </header>
